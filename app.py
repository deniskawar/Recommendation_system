...
def get_model_path(path: str) -> str:
    if os.environ.get("IS_LMS") == "1":  #checking whether it's executed in LMS or PC
...
def load_models(file_name):
    from_file = CatBoostClassifier() #load the model here
...
def get_id(id, limit=10, db: Session = Depends(get_db)):
...
def get_recommendations(id: int, time: datetime, limit: int = 5, db: Session = Depends(get_db)):
    ...
    print(pred_prob)
    print(post_text)
    probs_posts = pd.DataFrame(model.predict_proba(input_df), post_text['post_id'], columns=['to_drop', 'probs']).drop('to_drop', axis=1)
    #catboost
    print(probs_posts)
    
    sorted_probs_posts = probs_posts.sort_values('probs', ascending=False)
 
    print(sorted_probs_posts)
